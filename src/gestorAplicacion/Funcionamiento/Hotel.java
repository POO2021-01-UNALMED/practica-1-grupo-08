...
public class Hotel {
	...
	public void asignarHabitacion(Cliente cliente) {
		...
		if (Habitacion.disponibilidad(cliente.getNumAcompañantes())==true) {//va a disponibilidad con el parametro de num de clientes
			...
			}else if ((cliente.getNumAcompañantes()+1==3)) {
				Habitacion.setCapacidad3(cap3-1);
				
			}else if ((cliente.getNumAcompañantes()+1==4)) {
				Habitacion.setCapacidad4(cap4-1);
				
		    }else if ((cliente.getNumAcompañantes()+1==5)) {
				Habitacion.setCapacidad5(cap5-1);
			}  
		for(int i=0; i< Habitacion.habitaciones.size();i++) {
			if(Habitacion.habitaciones.get(i).getTipoCapacidad()==cliente.getNumAcompañantes()+1) {
